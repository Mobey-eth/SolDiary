...
contract CPAMMmobiTest {
    ...
    function mint(address _Lp, uint256 _shares) private {
        balanceOf[_Lp] += _shares;
        TotalSupply += _shares;
        emit Mint(_Lp, _shares);
    }

    function burn(address _Lp, uint256 _shares) private {
        balanceOf[_Lp] -= _shares;
        TotalSupply -= _shares;
        emit Burn(_Lp, _shares);
    }
